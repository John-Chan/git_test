...
int ep_create_or_exit()
{
    ...
    debug_loger.stream()<< "epoll fd = "<<fd<<EOL();
    return fd;
}
...
{
    debug_loger.stream()<< "epoll fd = "<<m_ep_fd<<EOL();
...
void ep_poller::update_channel( io_channel* channel )
{
    ...
    if(channel->reg_stat() == k_cnl_ctl_no_reg || channel->reg_stat()  == k_cnl_ctl_removed){
        ...
        channel->set_reg_stat(k_cnl_ctl_reged);
        m_registered_channels[channel->fd()]=channel;
...
void    ep_poller::do_update_channel(io_channel* channel,int32_t how)
{
    ...
    int fd=channel->fd();
    debug_loger.stream()<< "fd = "<<fd<<",how = "<<how <<",evt.events=" << evt.events << ",epoll fd = "<<m_ep_fd<<EOL();
    int32_t ret = epoll_ctl(m_ep_fd, how, fd, &evt);
    if(ret < 0){
...
{
	do_update_channel(channel,EPOLL_CTL_DEL);
    channel->set_reg_stat(k_cnl_ctl_removed);
#ifndef NDEBUG
	BOOST_ASSERT( m_registered_channels.find (channel->fd() ) != m_registered_channels.end ());
#endif
